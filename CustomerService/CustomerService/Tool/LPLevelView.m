...
- (void)setLevel:(float)level
{
    if (_level != level || level == 0) {
        ...
        //默认图标
        if (!_iconFull) {
            _iconFull = [UIImage imageNamed:@"O_xingon"];
            _iconHalf = [UIImage imageNamed:@"O_xingban"];
            _iconEmpty = [UIImage imageNamed:@"O_xing"];
...
- (void)scoreAnimation
{
    ...
    [UIView animateWithDuration:0.7 animations:^{
        self->animateLabel.center = CGPointMake(x, -40);
